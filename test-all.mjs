...
{
  // 1. Jurisdiction table exists and parses as YAML. This US-only fork
  //    removed upstream's only seeded row (CA-ON, non-US) rather than
  //    porting it — jurisdictions is deliberately empty until a sourced US
  //    row lands (see the header's contribution-rule discipline).
  ...
  if (!existsSync(alPath)) {
    ...
    try {
      ...
      const al = load(alRaw);
      if (al && typeof al.jurisdictions === 'object' && Object.keys(al.jurisdictions).length === 0) {
        pass('agency-licensing.yml parses; jurisdictions is empty (non-US CA-ON seed removed, no unsourced US row substituted) (#2037, US-only fork)');
      } else {
        fail(`agency-licensing.yml jurisdictions should be empty in this US-only fork, got: ${JSON.stringify(al?.jurisdictions)} (#2037)`);
      }
      if (
...
// --- offer-prep statutory-context notes for restrictive covenants (#2028) ---
{
  ...
  if (!existsSync(rcPath)) {
    ...
    try {
      ...
      // This US-only fork removed upstream's CA-ON row (non-US) rather than
      // porting it, so only the US-CA seed is required — the row count
      // itself should be exactly 1 (no CA-ON left, no unsourced US row
      // substituted in).
      ...
      if (
        ...
        pass('restrictive-covenants.yml parses; US-CA (§16600/§16600.5) non-compete seed complete, non-US CA-ON row removed (#2028, US-only fork)');
      } else {
        fail('restrictive-covenants.yml seed rows wrong for a US-only fork — expected exactly one complete US-CA non_compete row and no CA-ON row (#2028)');
      }
      if (
...
// Chinese-mode parity check removed: this is a US-only fork and modes/zh/
// (along with every other non-English modes/{lang}/ dir) was deleted, so
...
// MCP fallback — so the flag actually reaches the JD paths, not just scan/pipeline.
{
  ...
  if (langPipelines.length === 0) {
    pass('no language pipeline mirrors ship in this US-only fork — nothing to wire (#1449 Phase 2b, N/A)');
...
const criticalRoutingContracts = [
  ...
  // No 'language modes_dir override' entry here: this US-only fork ships no
  // modes/{lang}/ dirs, so a concrete language.modes_dir: modes/de example
  // would be misleading rather than documenting live behavior (see the
  // "Output Language vs Market Modes" section in AGENTS.md).
  ...
  if (!existsSync(pgPath)) {
    ...
    try {
      ...
      const completeRow = (r) =>
        ...
        Array.isArray(r.sources) && r.sources.length > 0 &&
        typeof r.as_of === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(r.as_of);
      // This US-only fork removed upstream's CA-ON and JP seed rows (both
      // non-US) rather than porting them, and deliberately did NOT
      // substitute an unsourced US-federal row — protected_grounds is empty
      // until a properly sourced EEOC row lands via PR (see the header's own
      // contribution-rule discipline, same policy as agency-licensing.yml).
      if (rows.length === 0) {
        pass('protected-grounds.yml parses; protected_grounds is empty (non-US CA-ON/JP seeds removed, no unsourced US row substituted) (#2030, US-only fork)');
      } else {
        fail(`protected-grounds.yml protected_grounds should be empty in this US-only fork, got ${rows.length} row(s): ${JSON.stringify(rows.map((r) => r?.jurisdiction))} (#2030)`);
      }
      if (
        ...
        pass('protected-grounds.yml header documents the contribution rule + not-legal-advice register and lists the US-federal EEOC candidate row as a comment only (#2030, US-only fork)');
      } else {
        fail('protected-grounds.yml header missing the contribution rule, not-legal-advice note, and/or the commented EEOC candidate row (#2030)');
      }
    } catch (e) {
  ...
  if (!existsSync(isPath)) {
    ...
    try {
      ...
      const completeRow = (r) =>
        ...
        typeof r.as_of === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(r.as_of);
      // This US-only fork removed upstream's CA-ON row (non-US) rather than
      // porting it — only the US §1324b seed is expected now.
      ...
      if (
        ...
        if (
          ...
          pass('immigration-status-requirements.yml parses with the verified US §1324b seed (only row — non-US CA-ON removed), non-empty lawful_screening_contrast, and the header contribution rule (#2033, US-only fork)');
        } else {
          fail('immigration-status-requirements.yml header missing the contribution rule (source + as_of + mandatory lawful_screening_contrast) (#2033)');
        }
      } else {
        fail('immigration-status-requirements.yml seed rows wrong for a US-only fork — expected exactly one complete US row (§1324b basis, both IER-approved questions in lawful_screening_contrast, government-contract + ITAR notes in exceptions, IER settlements + Facebook in enforcement_notes) and no CA-ON row (#2033)');
      }
    } catch (e) {
...
// --- jurisdiction-prohibited content signal (#2018): table + oferta Block G + apply Step 5c ---
{
  try {
    ...
    // This US-only fork removed upstream's CA-ON entry (non-US "Canadian
    // experience" requirement) rather than porting it — only US-CA remains.
    const caOn = byKey['CA-ON'];
    const usCa = byKey['US-CA'];
    if (
      entries.length === 1 && !caOn &&
      ...
      pass('jurisdiction-prohibited-content.yml parses with the verified US-CA seed entry (only entry — non-US CA-ON removed), sources, and the contribution rule (#2018, US-only fork)');
    } else {
      fail('jurisdiction-prohibited-content.yml seed entries wrong for a US-only fork — expected only US-CA (salary history, 2018-01-01) and no CA-ON entry (#2018)');
    }
  } catch (e) {
