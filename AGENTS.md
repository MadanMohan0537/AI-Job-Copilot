...
| `report-html.mjs` | Self-contained offline HTML dashboard (`reports/application-dashboard.html`) — inline SVG charts, no CDN/server, dependency-free alternative to `dashboard/`'s Go TUI. Adapted from ai-job-search's `/html-report` (MIT); reuses `stats.mjs`'s `computeTrackerStats`/`computeFunnel` so the two dashboards can't disagree. |
...
**This fork is US-only for market vocabulary.** Upstream career-ops ships market-specific mode sets (`modes/{lang}/`) supplying other countries' labor-market vocabulary (13. Monatsgehalt, kıdem tazminatı, etc.) — those dirs (German, French, Arabic, Japanese, Turkish, Hindi, and others) have been removed here, and `language.modes_dir` has nothing shipped to point at. All modes live directly in `modes/` (English/US labor-market concepts only). If you need another market's mode set, use upstream [career-ops](https://github.com/santifer/career-ops) instead.

`language.output` is a **separate, unaffected axis** — it still works exactly as upstream: it controls the language of human-facing prose (reports, cover letters, outreach) independent of market vocabulary, defaulting to English. See "Output Language vs Market Modes" below.
...
- `language.modes_dir` is read by upstream career-ops to select a market-vocabulary mode set (see above); this fork ships no non-English mode sets, so the key has no effect here.
...
| Wants a visual/offline dashboard of the pipeline (no Go installed, or just prefers a browser view) | `node report-html.mjs` — generates `reports/application-dashboard.html`, no server or Go toolchain needed |
