...
func TestCurrentIsAlwaysEnglish(t *testing.T) {
	// This fork is US/English-only: no SetLang/ToggleLang/GetLang, Current
	// is always &En.
	if Current != &En {
		t.Errorf("Current = %p; want &En (%p) — this fork has no other language catalog", Current, &En)
	}
}
